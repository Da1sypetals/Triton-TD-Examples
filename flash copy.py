...
def flash_attn_bwd_calc_delta(
    o: torch.Tensor,  # [batch_size, seq_len, num_heads, head_dim]
    do: torch.Tensor,  # [batch_size, seq_len, num_heads, head_dim]
):
...
class TritonFlashAttention(torch.autograd.Function):
    ...
    def forward(
        ...
        assert seq_len == num_k_tokens and num_k_tokens == num_v_tokens
        ...
        lse = torch.empty((batch_size, num_heads, seq_len), dtype=torch.float32, device=q.device)

        def grid(META):
            return (triton.cdiv(seq_len, META["BLOCK_SIZE_M"]), num_heads * batch_size, 1)
...
def main(batch_size, seq_len, num_heads, dim):
    ...
    q = torch.randn(batch_size, seq_len, num_heads, dim, dtype=torch.bfloat16, device="cuda")
    k = torch.randn(batch_size, seq_len, num_heads, dim, dtype=torch.bfloat16, device="cuda")
    v = torch.randn(batch_size, seq_len, num_heads, dim, dtype=torch.bfloat16, device="cuda")
    do = torch.randn(batch_size, seq_len, num_heads, dim, dtype=torch.bfloat16, device="cuda")
